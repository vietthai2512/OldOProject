...
public class TaoCauTangLienTuc extends TaoCauTang 
{
	...
	// Constructor
	public TaoCauTangLienTuc(List<CoPhieu> list, int soNgayLienTuc)
	{
		this.list = list;
	...
	{	
		...
		for (int i = 0; i <= list.size() - soNgayLienTuc; i++)
		{
			...
			if (lientuc) 
			{
				...
				tao_cau.append("Từ ").append(list.get(i).getNgay() + "/" + list.get(i).getThang()).append(" đến ").
									  ...
									  append(list.get(i).getTenCp() + " tăng liên tục. ");
		...
		if (khongco)
			tao_cau.append("Không có cổ phiếu nào tăng liên tục trong " + soNgayLienTuc + " ngày.");
